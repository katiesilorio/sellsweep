...
Overrides are expected and supported. Log per §17.3 (timestamp, item, destination, rule, reason). Do not re-argue a decision the seller has already made on a comparable item; apply the standing preference and note it.

**Raise a concern once, at the item where it first applies. Not on every subsequent item.**
